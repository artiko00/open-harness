# open-harness/linelens (Composer)
...
```bash
composer require --dev open-harness/linelens
```
...
```bash
vendor/bin/linelens check               # scan current directory
vendor/bin/linelens check --fail        # exit 1 on violations (CI / git hooks)
vendor/bin/linelens check --dir ./src   # scan a specific directory
vendor/bin/linelens check --max 200     # override the line limit
vendor/bin/linelens check --no-color    # plain output for logs
vendor/bin/linelens init                # generate a default linelens.json
vendor/bin/linelens version             # print version
```
...
```bash
# Husky pre-commit
vendor/bin/linelens check --fail
```
...
pre-commit:
  commands:
    linelens:
      run: vendor/bin/linelens check --fail --no-color
```
...
```bash
vendor/bin/linelens check               # escanea el directorio actual
vendor/bin/linelens check --fail        # exit 1 si hay violaciones (CI / git hooks)
vendor/bin/linelens check --dir ./src   # escanea un directorio específico
vendor/bin/linelens check --max 200     # sobrescribe el límite de líneas
vendor/bin/linelens check --no-color    # salida sin colores
vendor/bin/linelens init                # genera un linelens.json por defecto
vendor/bin/linelens version             # imprime la versión
```
